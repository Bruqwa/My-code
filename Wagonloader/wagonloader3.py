...
def init_wagon():
	...
	if wagon_length == 0: # Для упрощения ввода параметров заданы параметры по умолчанию
		global wagon
...
def init_boxes():
	...
	for i in range(num_boxes):
		i = Box(str(input('Box name: ')), 	# Ввод имени только английскими буквами из-за ограничений PIL
...
# Для упрощения ввода параметров заданы параметры по умолчанию или random
	if num_boxes == 0:
		...
		for r in range(50):
			c += 1
			bname = 'box' + str(c)
			boxes.append(Box(bname, random.randint(50, 100), random.randint(50, 100), 200, random.randint(5, 1000)))
	for b in boxes:
		print(b)
	return boxes
'''
...
def draw_boxes(boxes):
	...
	for b in boxes:
		...
		draw = ImageDraw.Draw(image)
		draw.rectangle((0, 0, b.length, b.width), fill=ImageColor.getrgb('green'), outline=ImageColor.getrgb('white'))
